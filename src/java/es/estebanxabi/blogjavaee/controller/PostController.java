package es.estebanxabi.blogjavaee.controller;

import es.estebanxabi.blogjavaee.entity.Categoria;
import es.estebanxabi.blogjavaee.entity.Post;
import es.estebanxabi.blogjavaee.view.util.JsfUtil;
import es.estebanxabi.blogjavaee.view.util.PaginationHelper;
import es.estebanxabi.blogjavaee.facade.PostFacade;
...
public class PostController implements Serializable {
    ...
    @EJB
    private es.estebanxabi.blogjavaee.facade.PostFacade ejbFacade;
    ...
    public List<Post> getItemsByPage(int page) {
        return ejbFacade.findPostByPage(page);
    }

    public int getNumMaxPages() {
        int numPostTotales = ejbFacade.findAll().size();
        int numTotalPaginas = (numPostTotales - 1) / 5;

        return numTotalPaginas;
    }

    public List<Post> getItemsByCategory(Categoria categoria, int page) {
        return ejbFacade.findPostBycategory(categoria, page);
    }

    public int getNumMaxPagesByCategory(Categoria categoria) {
        int numPostTotales = ejbFacade.findPostBycategory(categoria).size();
        int numTotalPaginas = (numPostTotales - 1) / 5;

        return numTotalPaginas;
    }

    public String getFechaFormateada(Post post) {
        Date fecha = post.getFecha();
        SimpleDateFormat formateador = new SimpleDateFormat("dd '/'MM'/'yyyy 'a las ' HH:MM", new Locale("es_ES"));
